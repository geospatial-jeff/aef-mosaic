...
impl CogReader {
    ...
    /// Prefetch all internal tiles needed for a set of pixel windows from a single COG.
    ///
    /// This method calculates which internal TIFF tiles are needed across all windows,
    /// deduplicates them, and fetches them all in a single batch request. The tiles
    /// are stored in the cache for later use by `read_window`.
    ///
    /// # Arguments
    /// * `tile` - The COG tile metadata
    /// * `windows` - List of pixel windows that will be read from this COG
    ///
    /// # Returns
    /// The number of internal tiles that were prefetched (not counting cache hits)
    pub async fn prefetch_tiles(&self, tile: &CogTile, windows: &[PixelWindow]) -> Result<usize> {
        if windows.is_empty() {
            return Ok(0);
        }
        ...
        // Calculate all internal tiles needed across all windows
        let mut tile_coords: std::collections::HashSet<(usize, usize)> = std::collections::HashSet::new();

        for window in windows {
            let start_tile_x = window.x / tile_width;
            let end_tile_x = (window.x + window.width.saturating_sub(1)) / tile_width;
            let start_tile_y = window.y / tile_height;
            let end_tile_y = (window.y + window.height.saturating_sub(1)) / tile_height;

            for ty in start_tile_y..=end_tile_y {
                for tx in start_tile_x..=end_tile_x {
                    tile_coords.insert((tx, ty));
                }
            }
        }
        ...
        let mut to_fetch: Vec<(usize, usize)> = Vec::new();
        {
            let cache = self.tile_cache.cache.read().await;
            for &(tx, ty) in &tile_coords {
                let key = (tile.s3_path.clone(), tx, ty);
        ...
        tracing::debug!(
            "Prefetching {} tiles from {} (out of {} needed, {} already cached)",
            to_fetch.len(),
            tile.tile_id,
            tile_coords.len(),
            tile_coords.len() - to_fetch.len()
        );

        // Register all tiles as in-flight (lock-free with DashMap)
        let mut senders: std::collections::HashMap<(usize, usize), broadcast::Sender<Result<Arc<TileArray>, String>>> = std::collections::HashMap::new();
        for &(tx, ty) in &to_fetch {
            let key = (tile.s3_path.clone(), tx, ty);
            if !self.tile_cache.in_flight.contains_key(&key) {
                let (sender, _) = broadcast::channel(64); // Increased buffer for high concurrency
                self.tile_cache.in_flight.insert(key, sender.clone());
                senders.insert((tx, ty), sender);
            }
        }

        // Only fetch tiles we registered (not already in-flight from another task)
        let coords_to_fetch: Vec<(usize, usize)> = senders.keys().copied().collect();

        if coords_to_fetch.is_empty() {
            // All tiles are being fetched by another task
            return Ok(0);
        }

        // Fetch all tiles in one batch
        ...
        let fetched_count = coords_to_fetch.len();

        // Handle results
        match batch_result {
            Ok(fetched_tiles) => {
                for (tile_data, &(tx, ty)) in fetched_tiles.into_iter().zip(coords_to_fetch.iter()) {
                    let key = (tile.s3_path.clone(), tx, ty);
                    let tile_arc = Arc::new(tile_data);
                    ...
                    // Notify any waiters
                    if let Some(sender) = senders.get(&(tx, ty)) {
                        let _ = sender.send(Ok(tile_arc));
                    }
            ...
            Err(e) => {
                ...
                for (tx, ty) in &coords_to_fetch {
                    if let Some(sender) = senders.get(&(*tx, *ty)) {
                        let _ = sender.send(Err(err_msg.clone()));
                    }
                }

                // Clean up in-flight (lock-free with DashMap)
                for (tx, ty) in &coords_to_fetch {
                    let key = (tile.s3_path.clone(), *tx, *ty);
